...
CONFIG="${CONFIG:-configs/openrlhf/hint_sft.yaml}"
NUM_GPUS="${NUM_GPUS:-1}"

# Unique wandb run name = YAML base name + timestamp, unless WANDB_RUN_NAME is set.
BASE_RUN_NAME="$(python3 -c "import yaml; c=yaml.safe_load(open('${CONFIG}')); print((c.get('logger',{}).get('wandb',{}) or {}).get('run_name','hint_controller_sft'))")"
